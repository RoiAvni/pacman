...
3. **Elastic Load Balancer (ELB)** – לחשיפה חיצונית של המשחק לציבור דרך כתובת IP קבוצתית  
4. **Node Group** – קבוצה מנוהלת של נודים שמריצים את הקונטיינרים באשכול  
5. **Terraform** – להקמת תשתיות באופן אוטומטי ומבוסס קוד (IaC)  
6. **S3 + DynamoDB** – לניהול ולאחסון מצב הריצה (`state`) של Terraform בפרויקטים  
7. **GitHub Actions** – אוטומציה על כל שינוי בענף `main` להפעלת תהליך CI/CD  
