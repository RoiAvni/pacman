...
3. **Elastic Load Balancer (ELB)** – קבוצת IP לחשיפה חיצונית של המשחק לציבור דרך כתובת  
4. **Node Group** – קבוצה מנוהלת של נודים שמריצים את הקונטיינרים באשכול  
5. **Terraform** – להקמת תשתיות באופן אוטומטי ומבוסס קוד (IaC)  
6. **S3 + DynamoDB** – ולאחסון מצב הריצה של Terraform (`state`) לצורך ניהול ועלייה מבוקרת בפרויקטים  
7. **GitHub Actions** – אוטומציה על כל שינוי בענף `main` להרצת תהליך CI/CD  
8. **kubectl** – פריסה, ניהול ובדיקת משאבים בתוך ה־Kubernetes Cluster
